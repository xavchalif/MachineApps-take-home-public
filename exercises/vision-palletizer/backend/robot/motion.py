...
class MotionController:
    ...
    def move_to_home(self) -> bool:
        """Move robot to a known safe home joint configuration."""
    ...
    def move_to_pick(
        ...
        """Execute approach -> descend -> grip -> retract for a pick."""
    ...
    def move_to_place(
        ...
        """Execute approach -> descend -> release -> retract for a place."""
        target = self._build_pose(position, orientation)
        approach = target.copy()
        approach[2] += self.APPROACH_HEIGHT_OFFSET
